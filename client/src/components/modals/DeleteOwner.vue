...
async function handleDeleteOwner() {
	if (!props.owner) {
		console.error("No owner to delete");
		return;
	}
	try {
		// Call the deleteOwner method from the store
		await ownerStore.deleteOwner(props.owner);
		console.log("Owner deleted successfully");
		await ownerStore.getOwners();
		await closeModal();
	} catch (error) {
		console.error("Error deleting owner: " + error.message);
		errorMessage.value = "Error deleting owner";
	}
}
</script>
